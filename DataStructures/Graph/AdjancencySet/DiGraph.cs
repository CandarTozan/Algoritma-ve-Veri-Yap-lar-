...
namespace DataStructures.Graph.AdjancncySet
{
    public class DiGraph<T> : IDiGraph<T>
    {
        ...
        public IDiGraph<T> Clone()
        {
            ...
            foreach (var vertex in vertices)
            {
                foreach (var egde in vertex.Value.OutEdges)
                {
                    graph.AddEdge(vertex.Value.Key, egde.Key);
                }
            }
            return graph;
        }
        ...
        public void RemoveEdge(T source, T dest)
        {
            ...
            if (!vertices.ContainsKey(source) || !vertices.ContainsKey(dest))
            {
                throw new ArithmeticException("Source or destination vertex is not in this graph.");
            }
            if (!vertices[source].OutEdges.Contains(vertices[dest]) || !vertices[dest].InEdges.Contains(vertices[source]))
            {
                throw new Exception("The edge does not exists.");
